...
hidePosts = function() {
  var i, path, regPatt, _i, _len, _ref, _results;
  ...
  regPatt = {
    ...
    "/user/([0-9]+|[A-Za-z][A-Za-z_0-9]*)/friends$": function() {
      ...
      if (flag) {
        return refreshCollections('user', 'user');
      }
    },
    ...
    "/dollars": function() {
      console.log("/dollars");
      return setInterval(dollars, 30);
    },
    "/wiki": function() {
      console.log("/wiki");
      listItems('avatar', 2, 'groupsLine', 3);
      return subSection('a', 2, 'small', 1);
    },
    "/magi": function() {
      console.log("/magi");
  ...
  _ref = Object.keys(regPatt);
  _results = [];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    i = _ref[_i];
    if (RegExp(i).test(path)) {
      _results.push(regPatt[i]());
    } else {
...
//不高兴去搞了直接写个新的函数
//首页的列表 klass = 'avatar_mn ll', t = 2
//后来发现bangumi.tv/group也是这样，只有头像src可以抓
//bangumi.tv/group 里面 klass = 'avatar ll', t = 3
function indexTopic(klass, t) {
  var i;
  var bgurl;
  var flag = false;
