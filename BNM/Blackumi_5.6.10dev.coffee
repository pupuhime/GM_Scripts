...
bkbackup = ->
  ...
  $('body').on 'click.bkm_import_omote', '#bkm_import_omote', (evt) ->
...
importOldData = (text) ->
  ...
  for i in avatars
    ...
    if ava[1] is "default"
      ava = [ava[0], "dft", "dft"]
    for j in [0...n] by 2
      if ava[0] is id1nid2[j]
        setData([id1nid2[j], id1nid2[j+1], ava[1], ava[2]])
  localStorage.clear()


importData = ->
  text = $("#ryououki").value.trim()
  if text is ""
    return false
