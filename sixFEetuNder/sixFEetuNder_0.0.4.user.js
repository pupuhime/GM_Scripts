...
var findFEN, getFirstChild, getTimeStamp, main, quickSort;
...
findFEN = function(interval) {
  ...
  for (_j = 0, _len = fener.length; _j < _len; _j++) {
    ...
    _results.push(posts[i].style = "background:#F09199;color:black;opacity:0.8;border-radius:5px;");
  }
  return _results;
};
...
function main () {
  try {
    if ($("#pageHeader a:first-child").attr("href").indexOf("/group/") === 0) {
      console.log("group topic");
      return findFEN(15897600000);
    }
  } catch (_error) {
